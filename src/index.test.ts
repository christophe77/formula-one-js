...
describe("Circuits API", () => {
  ...
  it("should retrieve all circuits for a year", async () => {
    ...
    expect(firstCircuit).toHaveProperty(
      "circuitName",
...
describe("Constructors API", () => {
  ...
  it("should retrieve all constructors for a specific race", async () => {
    const constructorList = await constructors.getConstructorsByRaceYear(
      2023,
      1
    );
    expect(constructorList).toBeDefined();
  ...
  it("should retrieve a specific constructor by ID", async () => {
    const constructor = await constructors.getConstructorInformations(
      "ferrari"
    );
...
describe("Finishing Status API", () => {
  ...
  it("should retrieve all finishing statuses for a year", async () => {
    const finishingStatusList =
      await finishingStatus.getFinishingStatusForSpecificYear(2023);
  ...
  it("should retrieve all finishing statuses for a specific race", async () => {
    const finishingStatusList =
      await finishingStatus.getFinishingStatusForSpecificYearRace(2023, 1);
...
describe("Standings API", () => {
  it("should retrieve constructor standings for a specific year", async () => {
    const constructorStandings = await standings.getConstructorStandingsByYear(
      2023
    );
    ...
    expect(constructor!.Constructor).toHaveProperty(
      "constructorId",
      "red_bull"
    );
  });
  ...
  it("should retrieve constructor standings for a specific race", async () => {
    const constructorStandings =
      await standings.getConstructorStandingsByYearRace(2023, 1);
  ...
  it("should retrieve driver standings for a specific race", async () => {
    const driverStandings = await standings.getDriverStandingsByYearRace(
      2023,
      1
    );
  ...
  it("should retrieve all championship constructor winners", async () => {
    const constructorWinners =
      await standings.getAllChampionshipConstructorWinners();
